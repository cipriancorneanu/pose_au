...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc31 = nn.Linear(1024, 10)

        self.fc12 = nn.Linear(512, 1024)
        self.fc22 = nn.Linear(1024, 1024)
        self.fc32 = nn.Linear(1024, 10)
    ...
    def forward(self, x1, x2)
        ...
        x1 = F.relu(self.fc11(z1))
        x1 = F.relu(self.fc21(x1))
        x1 = F.sigmoid(self.fc31(x1))

        x2 = F.relu(self.fc12(z2))
        x2 = F.relu(self.fc22(x2))
        x2 = F.sigmoid(self.fc32(x2))
...
def train(epoch):
    ...
    for iter, (data1, target, _) in enumerate(dl_train):
        data, target = data.cuda(), target.cuda()
        data, target = Variable(data).float(), Variable(target).float()

        optimizer.zero_grad()
        z1, z2, pred1, pred2 = model(data1. data1)
        
        loss = F.binary_cross_entropy(pred1, target) +
        F.binary_cross_entropy(pred2_target) + F.mse_loss(z1, z2)
